...
export default function SuiviRecrutementRedirectPage() {
    ...
    const rediriger = async () => {
      try {
        ...
        if (sessionsDistinctes.length > 0) {
          router.replace(`/mediation/rencontres-numeriques/actions-collectives/reponses/numerik-up/${encodeURIComponent(sessionsDistinctes[0])}`);
        } else {
          setIntrouvable(true);
        }
      } catch (error) {
