...
export default function ApprenantsRedirectPage() {
  ...
  useEffect(() => {
    const rediriger = async () => {
      try {
        const snapSessions = await getDoc(doc(db, "configuration_numerikup", "sessions"));
        ...
        if (sessionsDistinctes.length > 0) {
          router.replace(`/mediation/actions-collectives/reponses/numerik-up/${encodeURIComponent(sessionsDistinctes[0])}/apprenants`);
        } else {
          setIntrouvable(true);
        }
      } catch (error) {
  ...
  return (
    <PageGuard pageId="page_access_actions_collectives_accueil">
      <main className={`${quicksand.className} min-h-screen bg-[#F3F3F2] flex flex-col items-center justify-center gap-4 text-[#005259] p-8 text-center antialiased`}>
        {introuvable ? (
          <>
            <p className="font-bold uppercase tracking-widest text-xs">Aucune session Numérik'UP n'a encore été créée.</p>
            <Link
