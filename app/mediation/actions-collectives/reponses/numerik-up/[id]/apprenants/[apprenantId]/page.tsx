...
export default function FicheApprenantNumerikUpPage() {
  ...
  // Écriture directe sur le document Firestore de l'apprenant·e, distincte
  // du brouillon d'édition ci-dessus — mêmes mécanismes que les autres pages
  // de suivi (mise à jour optimiste locale + persistance en base).
  const mettreAJourChamp = async (champ: keyof Inscription, valeur: any) => {
    setInscription((prev) => (prev ? { ...prev, [champ]: valeur } : prev));
    try {
      await updateDoc(doc(db, "inscriptions_numerikup", apprenantId), { [champ]: valeur });
    } catch (error) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          <Section icon={AcademicCapIcon} titre="Suivi de recrutement">
            <div className="grid grid-cols-2 gap-4">
              ...
              <Champ label="Badge Pix" valeur={i.Pix_Badge} />
              <Champ label="Étoile Pix" valeur={i.Pix_Etoile} />
