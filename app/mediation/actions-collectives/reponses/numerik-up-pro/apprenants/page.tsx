...
export default function ApprenantsNumerikUpProRedirectPage() {
    ...
    const rediriger = async () => {
      try {
        ...
        if (sessionsDistinctes.length > 0) {
          router.replace(`/mediation/actions-collectives/reponses/numerik-up-pro/${encodeURIComponent(sessionsDistinctes[0])}/apprenants`);
        } else {
          setIntrouvable(true);
        }
      } catch (error) {
