...
export default function SuiviRecrutementPrfeRedirectPage() {
  ...
  useEffect(() => {
    const rediriger = async () => {
      try {
        const snapSessions = await getDoc(doc(db, "configuration_prfe", "sessions"));
        ...
        if (sessionsDistinctes.length > 0) {
          router.replace(`/mediation/actions-collectives/reponses/prfe/${encodeURIComponent(sessionsDistinctes[0])}`);
        } else {
          setIntrouvable(true);
        }
      } catch (error) {
  ...
  return (
    <PageGuard pageId="page_access_actions_collectives_accueil">
      <main className={`${quicksand.className} min-h-screen bg-[#F3F3F2] flex flex-col items-center justify-center gap-4 text-[#005259] p-8 text-center antialiased`}>
        {introuvable ? (
          <>
            <p className="font-bold uppercase tracking-widest text-xs">Aucune session PRFE n'a encore été créée.</p>
            <Link
