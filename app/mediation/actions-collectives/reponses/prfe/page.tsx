...
interface Inscription {
  ...
  // détaillé de sa session, sur /reponses/prfe/[id].
...
export default function ReponsesPrfePage() {
  ...
  const allerAuSuiviRecrutement = () => {
    ...
    router.push(`/mediation/actions-collectives/reponses/prfe/${encodeURIComponent(sessionsDistinctes[0])}`);
  };
  ...
  const basculerSuiviRecrutement = async (id: string, valeur: boolean) => {
    ...
    try {
      await updateDoc(doc(db, "inscriptions_prfe", id), { Suivi_Recrutement: valeur });
    } catch (error) {
  ...
  const supprimerInscription = async (i: Inscription) => {
    ...
    try {
      await deleteDoc(doc(db, "inscriptions_prfe", i.id));
    } catch (error) {
  ...
  const supprimerSelection = async () => {
    ...
    setSelection(new Set());
    try {
      await Promise.all(ids.map((id) => deleteDoc(doc(db, "inscriptions_prfe", id))));
    } catch (error) {
  ...
  const enregistrerEdition = async () => {
    ...
    try {
      await updateDoc(doc(db, "inscriptions_prfe", id), donnees);
      ...
      <div className="max-w-[100rem] mx-auto relative z-10 space-y-6">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center pb-4 border-b border-[#404040]/10 gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2.5 w-full lg:w-auto">
            ...
            {role === "admin" && (
              <Link
                href="/mediation/actions-collectives/reponses/prfe/importer"
