...
export default function AbsencesPrfeSessionPage() {
  ...
  const ajouterAbsence = async () => {
    ...
    try {
      await updateDoc(doc(db, "inscriptions_prfe", apprenant.id), { Absences: nouvelleListe });
    } catch (error) {
  ...
  const enregistrerModificationAbsence = async () => {
    ...
    try {
      await updateDoc(doc(db, "inscriptions_prfe", apprenantId), { Absences: nouvelleListe });
    } catch (error) {
