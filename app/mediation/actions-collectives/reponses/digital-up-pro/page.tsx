...
import { HomeIcon, MagnifyingGlassIcon, ClipboardDocumentCheckIcon, DocumentArrowUpIcon, TrashIcon, DocumentDuplicateIcon, ChevronUpIcon, ChevronDownIcon, ChevronUpDownIcon, PencilSquareIcon, XMarkIcon, CheckIcon, LanguageIcon } from "@heroicons/react/24/outline";
...
interface Inscription {
  ...
  // détaillé de sa session, sur /reponses/digital-up-pro/[id].
...
export default function ReponsesDigitalUpProPage() {
  ...
  const allerAuSuiviRecrutement = () => {
    ...
    router.push(`/mediation/actions-collectives/reponses/digital-up-pro/${encodeURIComponent(sessionsDistinctes[0])}`);
  };
  ...
  const basculerSuiviRecrutement = async (id: string, valeur: boolean) => {
    ...
    try {
      await updateDoc(doc(db, "inscriptions_digitaluppro", id), { Suivi_Recrutement: valeur });
    } catch (error) {
  ...
  const supprimerInscription = async (i: Inscription) => {
    ...
    try {
      await deleteDoc(doc(db, "inscriptions_digitaluppro", i.id));
    } catch (error) {
  ...
  const confirmerDuplication = async () => {
    ...
    try {
      await addDoc(collection(db, "inscriptions_digitaluppro"), {
        ...champs,
        Session: sessionCible,
        Suivi_Recrutement: false,
        createdAt: serverTimestamp(),
      });
      const snap = await getDocs(query(collection(db, "inscriptions_digitaluppro"), orderBy("createdAt", "desc")));
  ...
  const supprimerSelection = async () => {
    ...
    setSelection(new Set());
    try {
      await Promise.all(ids.map((id) => deleteDoc(doc(db, "inscriptions_digitaluppro", id))));
    } catch (error) {
  ...
  const enregistrerEdition = async () => {
    ...
    try {
      await updateDoc(doc(db, "inscriptions_digitaluppro", id), donnees);
      ...
      <div className="max-w-[100rem] mx-auto relative z-10 space-y-6">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center pb-4 border-b border-[#404040]/10 gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2.5 w-full lg:w-auto">
            ...
            {role === "admin" && (
              <Link
                href="/mediation/actions-collectives/reponses/digital-up-pro/importer"
            ...
            <Link
              href="/mediation/actions-collectives/reponses/digital-up-pro/test-langue"
              className="flex items-center gap-2 bg-white hover:bg-[#005259] hover:text-white border border-[#404040]/10 px-3.5 py-2 rounded-xl text-[#005259] transition-all text-xs font-bold uppercase tracking-wider shadow-sm"
            >
              <LanguageIcon className="w-4 h-4 text-[#EA601F]" />
              <span>Test de langue</span>
            </Link>
            <Link
              href="/"
              className="flex items-center gap-2 bg-white hover:bg-[#005259] hover:text-white border border-[#404040]/10 px-3.5 py-2 rounded-xl text-[#005259] transition-all text-xs font-bold uppercase tracking-wider shadow-sm"
