...
export default function EvolutionDigitalUpProSessionPage() {
  ...
  // Code interne de la session (ex. "MN26_DUPPRO-91_01"), généré depuis la
  ...
  const mettreAJourCase = async (id: string, cle: string, valeur: string) => {
    ...
    if (valeur === "AB") {
      ...
      try {
        ...
        await updateDoc(doc(db, "inscriptions_digitaluppro", id), champs);
      } catch (error) {
  ...
  const mettreAJourCaseGroupe = async (cle: string, valeur: string) => {
    ...
    try {
      await Promise.all(concernes.map((a) => {
        ...
        const absencesMaj = absencesParId.get(a.id);
        if (absencesMaj) champs.Absences = absencesMaj;
        return updateDoc(doc(db, "inscriptions_digitaluppro", a.id), champs);
      }));
    } catch (error) {
  ...
  const mettreAJourRetard = async (id: string, cle: string, valeur: string) => {
    ...
    try {
      await updateDoc(doc(db, "inscriptions_digitaluppro", id), { [`Evolution_Retards.${cle}`]: valeur });
    } catch (error) {
  ...
  const basculerActif = async (id: string, valeur: boolean) => {
    ...
    try {
      await updateDoc(doc(db, "inscriptions_digitaluppro", id), { Evolution_Actif: valeur });
    } catch (error) {
      console.error("Erreur lors de la mise à jour de l'évolution :", error);
    }
  };
      ...
      <div className="max-w-[100rem] mx-auto relative z-10 space-y-6">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center pb-4 border-b border-[#404040]/10 gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2.5 w-full lg:w-auto">
            ...
            <Link
              href={`/mediation/actions-collectives/reponses/digital-up-pro/${encodeURIComponent(sessionId)}/absences`}
            ...
            {permissions?.role === "admin" && (
              <Link
                href="/mediation/actions-collectives/inscription/digital-up-pro/parametres"
                className="flex items-center gap-2 bg-white hover:bg-[#005259] hover:text-white border border-[#404040]/10 px-3.5 py-2 rounded-xl text-[#005259] transition-all text-xs font-bold uppercase tracking-wider shadow-sm"
