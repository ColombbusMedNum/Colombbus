...
export default function AbsencesDigitalUpProSessionPage() {
  ...
  const ajouterAbsence = async () => {
    ...
    try {
      await updateDoc(doc(db, "inscriptions_digitaluppro", apprenant.id), { Absences: nouvelleListe });
    } catch (error) {
  ...
  const enregistrerModificationAbsence = async () => {
    ...
    try {
      await updateDoc(doc(db, "inscriptions_digitaluppro", apprenantId), { Absences: nouvelleListe });
    } catch (error) {
