...
function mapperLigne(ligne: string[], colonnes: Record<string, number[]>): LigneImportee {
  ...
  // Le formulaire Digital Up 96H n'a pas de question générique "structure
  // d'accompagnement" à choix multiple — juste une question Oui/Non dédiée au
  // PLIE. Une réponse "Oui" vaut donc structure = "PLIE", sauf si une colonne
  // structure générique (autre export) est déjà renseignée.
  const pliePresent = normaliser(valeur(ligne, colonnes.plie)).startsWith("oui");
  const structureAccompagnement = structureConnue || (pliePresent ? "PLIE" : structureBrute ? "Autre" : "");
  const structureAutre = structureConnue || pliePresent ? "" : structureBrute;

  const franceTravail = normaliserOuiNon(valeur(ligne, colonnes.franceTravail));

  // Le formulaire Digital Up 96H ne pose pas de question "Territoire" —
  // celui-ci se déduit des deux premiers chiffres du code postal (numéro de
  // département), une colonne "territoire" explicite (autre export) restant
  // prioritaire si présente.
  const codePostal = valeur(ligne, colonnes.codePostal).trim();
  const territoire = valeur(ligne, colonnes.territoire).trim() || codePostal.slice(0, 2);
  ...
  return {
    ...
    Positionnement_Initial: valeur(ligne, colonnes.positionnement),
...
function detecterColonnes(entetes: string[]): Record<string, number[]> {
  return {
    ...
    // Question dédiée du formulaire Digital Up 96H ("... accompagné.e par le
    // PLIE ?"), distincte de la question générique "structure
    // d'accompagnement" ci-dessus — voir le repli PLIE dans mapperLigne.
    plie: trouverColonnes(entetes, /accompagne.*plie/),
    positionnement: trouverColonnes(entetes, /positionnement du beneficiaire/),
    ...
    // Le formulaire Digital Up 96H ne pose pas "session souhaitée" mais
    // "Quelles sont vos disponibilités pour l'information collective ? /
    // Choix de session n°X" (une colonne par branche du formulaire) — les deux
    // formulations sont acceptées pour ne pas casser d'autres exports.
    session: trouverColonnes(entetes, /disponibilit.*information collective|choix de session|session souhait/),
...
export default function ImporterDigitalUpProPage() {
    ...
    try {
      ...
      for (let i = 0; i < lignesParsees.length; i += taillePaquet) {
        ...
        paquet.forEach((l) => {
          ...
          const ref = doc(collection(db, "inscriptions_digitaluppro"));
          batch.set(ref, {
            ...donnees,
      ...
      console.error("Erreur lors de l'import des préinscriptions DIGITAL UP 96H :", error);
