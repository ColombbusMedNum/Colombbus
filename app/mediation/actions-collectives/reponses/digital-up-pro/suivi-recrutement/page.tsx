...
export default function SuiviRecrutementDigitalUpProRedirectPage() {
    ...
    const rediriger = async () => {
      try {
        ...
        if (sessionsDistinctes.length > 0) {
          router.replace(`/mediation/actions-collectives/reponses/digital-up-pro/${encodeURIComponent(sessionsDistinctes[0])}`);
        } else {
          setIntrouvable(true);
        }
      } catch (error) {
