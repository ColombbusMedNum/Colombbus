...
// par la page des préinscriptions pour accéder à la liste des apprenant·e·s
// (même mécanisme que suivi-recrutement/page.tsx, cible différente).
export default function ApprenantsDigitalUpProRedirectPage() {
    ...
    const rediriger = async () => {
      try {
        ...
        if (sessionsDistinctes.length > 0) {
          router.replace(`/mediation/actions-collectives/reponses/digital-up-pro/${encodeURIComponent(sessionsDistinctes[0])}/apprenants`);
        } else {
          setIntrouvable(true);
        }
      } catch (error) {
        console.error("Erreur lors de la redirection vers les apprenant·e·s :", error);
        setIntrouvable(true);
      }
    };
    rediriger();
    ...
    <PageGuard pageId="page_access_actions_collectives_accueil">
      <main className={`${quicksand.className} min-h-screen bg-[#F3F3F2] flex flex-col items-center justify-center gap-4 text-[#005259] p-8 text-center antialiased`}>
        {introuvable ? (
          ...
          <p className="font-bold animate-pulse tracking-widest text-xs uppercase">Redirection vers les apprenant·e·s...</p>
        )}
      </main>
    </PageGuard>
  );
