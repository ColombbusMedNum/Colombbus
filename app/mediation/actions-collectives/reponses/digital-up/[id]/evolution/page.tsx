...
// journal des absences ci-dessous, pour ne pas avoir à ressaisir le même
// évènement à la main. Une entrée déjà existante pour cette date est mise à
// jour (justifiée/non justifiée) plutôt que dupliquée ; retourne null si rien
// à changer (autre code, ou déjà à jour). Changer vers un autre code ensuite
// ne supprime pas l'entrée — à retirer à la main en cas d'erreur de saisie.
...
export default function EvolutionDigitalUpSessionPage() {
  ...
  const mettreAJourCase = async (id: string, cle: string, valeur: string) => {
    ...
    if (valeur === "AB") {
      ...
      try {
        ...
        await updateDoc(doc(db, "inscriptions_digitalup", id), champs);
      } catch (error) {
  ...
  const mettreAJourCaseGroupe = async (cle: string, valeur: string) => {
    ...
    try {
      await Promise.all(concernes.map((a) => {
        ...
        const absencesMaj = absencesParId.get(a.id);
        if (absencesMaj) champs.Absences = absencesMaj;
        return updateDoc(doc(db, "inscriptions_digitalup", a.id), champs);
      }));
    } catch (error) {
  ...
  const mettreAJourRetard = async (id: string, cle: string, valeur: string) => {
    ...
    try {
      await updateDoc(doc(db, "inscriptions_digitalup", id), { [`Evolution_Retards.${cle}`]: valeur });
    } catch (error) {
  ...
  const basculerActif = async (id: string, valeur: boolean) => {
    ...
    try {
      await updateDoc(doc(db, "inscriptions_digitalup", id), { Evolution_Actif: valeur });
    } catch (error) {
      console.error("Erreur lors de la mise à jour de l'évolution :", error);
    }
  };
  ...
  const ajouterAbsence = async () => {
    ...
    try {
      await updateDoc(doc(db, "inscriptions_digitalup", apprenant.id), { Absences: nouvelleListe });
    } catch (error) {
  ...
  const enregistrerModificationAbsence = async () => {
    ...
    try {
      await updateDoc(doc(db, "inscriptions_digitalup", apprenantId), { Absences: nouvelleListe });
    } catch (error) {
      ...
      <div className="max-w-[100rem] mx-auto relative z-10 space-y-6">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center pb-4 border-b border-[#404040]/10 gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2.5 w-full lg:w-auto">
            <Link
              href={`/mediation/actions-collectives/reponses/digital-up/${encodeURIComponent(sessionId)}/apprenants`}
