...
export default function ParametresNumerikUpProPage() {
  ...
  // numerik-up-pro), choisis parmi la bibliothèque partagée (voir
  ...
  // présence, voir reponses/numerik-up-pro/[id]/evolution) — modifiables
  ...
  const sauvegarderCategoriesEvolution = async (liste: CategorieEvolution[]) => {
    ...
    await setDoc(doc(db, "configuration_numerikuppro", "evolutionCategories"), { liste });
  };
  ...
  const basculerLogo = async (territoire: string, logoId: string) => {
    ...
    await setDoc(doc(db, "configuration_numerikuppro", "logosFormulaire"), { parTerritoire: misesAJour });
  };

  const sauvegarderProgrammes = async (parParcours: Record<string, { storagePath: string; url: string }[]>) => {
    setProgrammes(parParcours);
    await setDoc(doc(db, "configuration_numerikuppro", "programmes"), { parParcours });
  };
  ...
  const televerserProgramme = async (parcoursId: string, fichier: File) => {
    ...
    try {
      const storagePath = `programmes_inscription/numerik-up-pro/${parcoursId}/${Date.now()}_${fichier.name}`;
      ...
      <div className="max-w-5xl mx-auto relative z-10 space-y-6">
        ...
        <div className="bg-white border border-[#404040]/10 rounded-2xl p-5 shadow-sm space-y-3">
          ...
          <p className="text-[10px] text-[#404040]/50">
            ...
            <a href="/inscription/numerik-up-pro" target="_blank" rel="noopener noreferrer" className="underline hover:text-[#005259]">la version publique du formulaire</a>. Un partenaire ne finance pas forcément l'action sur tous les territoires : la sélection se fait territoire par territoire.
          </p>
