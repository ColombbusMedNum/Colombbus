...
export default function ParametresPrfePage() {
  ...
  // présence, voir reponses/prfe/[id]/evolution) — modifiables depuis cette
  // page, contrairement aux 4 codes structurels fixes (Absence justifiée/non
  ...
  const sauvegarderCategoriesEvolution = async (liste: CategorieEvolution[]) => {
    ...
    await setDoc(doc(db, "configuration_prfe", "evolutionCategories"), { liste });
  };
  ...
  const ajouterParcours = async () => {
    ...
    await setDoc(doc(db, "configuration_prfe", "parcours"), { liste: misesAJour });
  };
  ...
  const ajouterTerritoire = async () => {
    ...
    await setDoc(doc(db, "configuration_prfe", "territoires"), { liste: misesAJour });
  };
  ...
  // Code interne "MN{AA}_PRFE-{territoire}_{NN}" — jamais affiché sur le
  ...
  const genererCode = async (ligne: { parcoursId: string; territoire: string; date: string }) => {
    ...
    const code = `MN${annee}_PRFE-${ligne.territoire}_${numero}`;
  ...
  if (role !== "admin") {
    return (
      <div className={`${quicksand.className} min-h-screen bg-[#F3F3F2] flex flex-col items-center justify-center gap-4 text-center p-8 antialiased`}>
        ...
        <Link
          href="/mediation/actions-collectives/inscription/prfe"
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl md:text-3xl font-bold uppercase text-[#005259] tracking-tight">
                Gérer <span className="text-[#EA601F] font-semibold">Préparation Parcours Métiers</span>
              </h1>
