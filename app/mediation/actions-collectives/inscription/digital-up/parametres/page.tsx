...
// générique par défaut, librement modifiable ensuite depuis cette page.
...
export default function ParametresDigitalUpPage() {
  ...
  const [nouvelleSessionParcours, setNouvelleSessionParcours] = useState("digital-up");
  ...
  useEffect(() => {
    const charger = async () => {
      const [snapSessions, snapParcours, snapTerritoires, snapCategoriesEvolution] = await Promise.all([
  ...
  // présence, voir reponses/digital-up/[id]/evolution) — modifiables depuis
  ...
  const sauvegarderCategoriesEvolution = async (liste: CategorieEvolution[]) => {
    ...
    await setDoc(doc(db, "configuration_digitalup", "evolutionCategories"), { liste });
  };
  ...
  const sauvegarderSessions = async (parTerritoire: Record<string, Record<string, string[]>>, codesActuels: Record<string, string>) => {
    await setDoc(doc(db, "configuration_digitalup", "sessions"), { parTerritoire, codes: codesActuels });
  };
  ...
  const ajouterParcours = async () => {
    ...
    await setDoc(doc(db, "configuration_digitalup", "parcours"), { liste: misesAJour });
  };
  ...
  const ajouterTerritoire = async () => {
    ...
    await setDoc(doc(db, "configuration_digitalup", "territoires"), { liste: misesAJour });
  };
  ...
  // Code interne "MN{AA}_DIGUP-{territoire}_{NN}" — jamais affiché sur le
  ...
  const genererCode = async (ligne: { parcoursId: string; territoire: string; date: string }) => {
    ...
    const code = `MN${annee}_DIGUP-${ligne.territoire}_${numero}`;
  ...
  if (role !== "admin") {
    return (
      <div className={`${quicksand.className} min-h-screen bg-[#F3F3F2] flex flex-col items-center justify-center gap-4 text-center p-8 antialiased`}>
        ...
        <Link
          href="/mediation/actions-collectives/inscription/digital-up"
      ...
      <div className="max-w-5xl mx-auto relative z-10 space-y-6">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center pb-4 border-b border-[#404040]/10 gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl md:text-3xl font-bold uppercase text-[#005259] tracking-tight">
                Gérer <span className="text-[#EA601F] font-semibold">Digital'UP</span>
              </h1>
          ...
          <div className="flex flex-wrap items-center gap-2.5 w-full lg:w-auto">
            ...
            >
              <ArrowLeftIcon className="w-4 h-4 text-[#EA601F]" />
              <span>Formulaire Digital'UP</span>
            </Link>
            <Link
              href="/"
              className="flex items-center gap-2 bg-white hover:bg-[#005259] hover:text-white border border-[#404040]/10 px-3.5 py-2 rounded-xl text-[#005259] transition-all text-xs font-bold uppercase tracking-wider shadow-sm"
