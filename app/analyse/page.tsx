...
import { PAGES_CATALOG, DEFAULT_PERMISSIONS, ALL_ACTION_IDS, ActionItem } from "../../lib/permissionsCatalog";
                        ...
                        <tbody className="divide-y divide-[#404040]/10">
                          {page.actions.map((action) => (
                            <tr key={action.id} className="hover:bg-[#F3F3F2]/50 transition-colors group">
                              ...
                              {ROLES.map((role) => {
                                const estCoche = role.id === "admin" ? true : !!matrix[action.id]?.[role.id];
                                return (
