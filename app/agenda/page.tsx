...
// Fonction utilitaire pour convertir un Hex (#RRGGBB) en RGBA
...
export default function PlanningExpertMix() {
  ...
  const [filtrePersoUniquement, setFiltrePersoUniquement] = useState(true);

  // Référence pour fermer la cloche de notification au clic extérieur
  const notifRef = useRef<HTMLDivElement>(null);

  // État pour la gestion des commentaires
      ...
      {isActiviteModalOpen && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center z-[110] p-4">
          <form onSubmit={handleSaveActiviteType} className="bg-slate-900 border border-slate-800 p-5 rounded-xl w-full max-w-xs space-y-3">
            ...
            {/* CHAMP SAISIE MANUELLE DU NOM DE L'ACTIVITÉ */}
            <div className="flex flex-col gap-1">
              <label className="text-[10px] text-slate-400 font-medium">Nom de l'activité</label>
              <input 
                required
                placeholder="Ex: RN Suresnes, Bureau, Congés..." 
                value={newActivite.lieu} 
                className="field-dark font-medium" 
                onChange={e => setNewActivite({...newActivite, lieu: e.target.value})} 
              />
            </div>

            {/* OPTION : SELECTIONNER UN LIEU EXISTANT POUR PRÉ-REMPLIR L'ADRESSE ET LE TERRITOIRE UNIQUEMENT */}
            {localisations && localisations.length > 0 && (
              <div className="flex flex-col gap-1">
                <label className="text-[10px] text-slate-500 font-medium">Pré-remplir l'adresse (Optionnel)</label>
                <select 
                  className="field-dark text-slate-400"
                  value=""
                  onChange={(e) => {
                    ...
                    if (locFound) {
                      const adresseComplete = `${locFound.adresse || ""}, ${locFound.codePostal || ""} ${locFound.localisation || ""}`.trim();
                ...
                >
                  <option value="" className="bg-slate-900 text-slate-400">-- Choisir une adresse enregistrée --</option>
                  {localisations.map((loc) => (
                    <option key={loc.id} value={loc.nomRaccourci || loc.nomComplet} className="bg-slate-900 text-white">
                      {loc.nomRaccourci} {loc.nomComplet ? `(${loc.nomComplet})` : ""}
                    </option>
                  ))}
                </select>
              </div>
            ...
            <div className="flex flex-col gap-1">
              <label className="text-[10px] text-slate-400 font-medium">Adresse</label>
              <input 
                placeholder="Ex: Hôtel de Ville, Suresnes..." 
                value={newActivite.adresse} 
                className="field-dark" 
                onChange={e => setNewActivite({...newActivite, adresse: e.target.value})}
              />
            </div>
