...
export default function PlanningSuresnes() {
  ...
  const handleDemandeSpecifiqueChange = async (creneauId: string, valeur: string) => {
    try {
      await updateDoc(doc(db, "planning_suresnes", creneauId), {
        demandeSpecifique: valeur
      });
    } catch (error) {
      console.error("Erreur lors de la mise à jour de la demande spécifique :", error);
    }
  };
      ...
      <div className="max-w-[95%] mx-auto px-2">
        ...
        <div className="space-y-6">
          {days.map((day, i) => {
            ...
            if (entries.length === 0) {
              if (filterTodayOnly && dateStr === todayStr) {
                return (
                  <div key={i} className="bg-slate-900/40 border border-dashed border-slate-800 rounded-xl p-8 text-center text-slate-500 text-sm font-medium">
                    Aucun rendez-vous de planifié pour aujourd'hui.
                  </div>
                );
              }
              return null;
            ...
            return (
              <div key={i} className={`bg-slate-900/60 border rounded-xl overflow-hidden shadow-xl backdrop-blur-sm transition-all ${dateStr === todayStr ? 'border-emerald-500/40 shadow-emerald-950/10' : 'border-slate-800'}`}>
                ...
                <div className="bg-slate-950 px-5 py-3 border-b border-slate-800 flex items-center justify-between">
                  ...
                  {filterTodayOnly && (
                    <span className="flex items-center gap-1 text-[10px] bg-emerald-950 text-emerald-400 font-bold tracking-wider uppercase px-2 py-0.5 rounded border border-emerald-900/50">
                      <AdjustmentsHorizontalIcon className="w-3 h-3" /> Filtre actif
                    </span>
                  )}
                </div>
                              ...
                              >
                                ...
                                <div className="xl:col-span-1 text-left xl:text-right shrink-0">
                                  {isOrphan ? (
                                    ...
                                    <span className="inline-block text-center w-full text-slate-500 bg-slate-950 border border-slate-800 px-2 py-1 rounded-md text-[9px] font-bold tracking-wider uppercase0">
                                      À attribuer
                                    </span>
                                  ) : estEnregistreDansLaFiche ? (
                                    <Link 
                                      href={`/liste-beneficiaires/${bTrouve.id}`}
                                      className="inline-flex items-center justify-center gap-1 w-full text-emerald-400 bg-emerald-950/30 border border-emerald-900/40 hover:border-emerald-500/60 px-2 py-1 rounded-md text-[9px] font-bold tracking-wider uppercase transition-all shadow-sm"
                                      title="Visite pointée — Cliquer pour aller sur la fiche"
                                    >
                                    ...
                                    <Link 
                                      ...
                                      title="Pas encore de suivi aujourd'hui — Cliquer pour aller sur la fiche"
                                    >
